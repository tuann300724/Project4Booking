...
const PromoWheel = ({ isOpen, onClose}) => {
  ...
  const checkSpinEligibility = useCallback(() => {
    ...
    const now = new Date();
    const secondsPassed = (now - lastSpinDate) / 1000;
    const secondsUntilNextSpin = Math.max(
      0,
      Math.ceil(COOLDOWN_SECONDS - secondsPassed)
    );
  ...
  const spinWheel = () => {
    ...
    if (Math.random() < 0.1) {
      ...
      const otherPrizes = [...Array(prizes.length).keys()].filter(
        (i) => i !== BETTER_LUCK_INDEX
      );
              ...
              <div className="mb-4">
                <div className="w-full bg-gray-200 rounded-full h-2.5">
                  <div
                    ...
                    style={{
                      width: `${(countdown / COOLDOWN_SECONDS) * 100}%`,
                    }}
                  ></div>
                </div>
              </div>
